...
def generate_dynamic_html_structure(title: str, short_summary: str, research_content: str, key_points: list, image_path: str = None, image_metadata: dict = None) -> dict:
    ...
    html_parts = {
        ...
        'body': {
            ...
            'article_image': _generate_article_image(image_path, image_metadata) if image_path else '',
...
def _generate_article_image(image_path: str, image_metadata: dict = None) -> str:
    """Generate the article image section."""
    if not image_path or not os.path.exists(image_path):
        return ""
    
    # Get just the filename for HTML (no directory path)
    # This ensures the image reference works when HTML and image are in the same directory
    image_filename = os.path.basename(image_path)
    
    # Debug: print what we're working with
    print(f"🔍 Image path received: {image_path}")
    print(f"🔍 Using filename in HTML: {image_filename}")
    
    # Create caption from metadata if available
    caption = ""
    if image_metadata and image_metadata.get('revised_prompt'):
        # Use the revised prompt as caption, but keep it short
        full_prompt = image_metadata['revised_prompt']
        # Truncate if too long
        if len(full_prompt) > 100:
            caption = full_prompt[:97] + "..."
        else:
            caption = full_prompt
    
    return f"""
        <div class="article-image">
            <img src="{image_filename}" alt="Article illustration" loading="lazy">
            {f'<div class="image-caption">{caption}</div>' if caption else ''}
            <div class="image-credit">Generated by Azure OpenAI DALL-E 3</div>
        </div>
    """
...
def assemble_html_document(html_parts: dict) -> str:
    ...
    if html_parts['body']['article_image']:
        html_doc.append(html_parts['body']['article_image'])
...
image_path: str | Optional path to the generated article image
image_metadata: dict | Optional metadata about the generated image""",
